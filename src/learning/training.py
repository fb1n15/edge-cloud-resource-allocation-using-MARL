...
from learning.models.centralised_fc_model import CentralisedModelFC
...
def train(config):
    ...
    analysis = tune.run(
        config["trainer"],  # the model to train
        name=config["name"],  # name of the experiment
        scheduler=scheduler,  # Scheduler for executing the experiment
        config=trainer_config,
        # algorithm-specific configuration (e.g., num_workers)
        ...
        # reuse_actors=True,
        local_dir="results/",  # directory to save training results to
        verbose=3,
        # Verbosity mode (e.g., 3 = status and detailed trial results)
        checkpoint_at_end=False,
        # whether to checkpoint at the end of the experiment
        num_samples=config.get("samples", 1),  # to take multiple random samples
        # fail_fast='raise'  # whether to fail upon the first error, should be used with caution
    )
